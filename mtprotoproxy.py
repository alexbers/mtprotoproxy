...
# expiration
USER_EXPIRARION = config.get("USER_EXPIRARION", {})
...
async def handle_client(reader_clt, writer_clt):
    ...
    if ((user not in USER_MAX_TCP_CONNS or stats[user]["curr_connects"] <= USER_MAX_TCP_CONNS[user]) and (user not in USER_EXPIRARION or datetime.datetime.now() <= datetime.datetime.strptime(USER_EXPIRARION[user],"%d/%m/%Y"))):
