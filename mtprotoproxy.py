...
async def handle_fake_tls_handshake(handshake, reader, writer, peer):
    ...
    for user in config.USERS:
        ...
        accept_bad_time = (
            config.IGNORE_TIME_SKEW or is_time_skewed or client_time_is_small
        )
...
async def handle_client(reader_clt, writer_clt):
    set_keepalive(
        writer_clt.get_extra_info("socket"), config.CLIENT_KEEPALIVE, attempts=3
    )
    ...
    tg_to_clt = tg_connect_reader_to_writer(
        reader_tg, writer_clt, user, get_to_clt_bufsize(), False
    )
    clt_to_tg = tg_connect_reader_to_writer(
        reader_clt, writer_tg, user, get_to_tg_bufsize(), True
    )
    ...
    tcp_limit_hit = (
        user in config.USER_MAX_TCP_CONNS
        and user_stats[user]["curr_connects"] > config.USER_MAX_TCP_CONNS[user]
    )

    user_expired = (
        user in config.USER_EXPIRATIONS
        and datetime.datetime.now() > config.USER_EXPIRATIONS[user]
    )

    user_data_quota_hit = user in config.USER_DATA_QUOTA and (
        user_stats[user]["octets_to_client"] + user_stats[user]["octets_from_client"]
        > config.USER_DATA_QUOTA[user]
    )
...
async def get_encrypted_cert(host, port, server_name):
    ...
    if len(record3) < MIN_CERT_LEN:
        ...
        if record4_type != 23:
            return b""
        msg = (
            "The MASK_HOST %s sent some TLS record before certificate record, this makes the "
            + "proxy more detectable"
        ) % config.MASK_HOST
        print_err(msg)
    ...
    while True:
        try:
            ...
            task = get_encrypted_cert(
                config.MASK_HOST, config.MASK_PORT, config.TLS_DOMAIN
            )
...
def print_tg_info():
    ...
    if config.PORT == 3256:
        ...
        if not config.MODES["classic"] and not config.MODES["secure"]:
            print(
                "Since you have TLS only mode enabled the best port is 443", flush=True
            )
...
def create_servers(loop):
    ...
    if config.LISTEN_UNIX_SOCK and has_unix:
        ...
        task = asyncio.start_unix_server(
            handle_client_wrapper, config.LISTEN_UNIX_SOCK, limit=get_to_tg_bufsize()
        )
        servers.append(loop.run_until_complete(task))
    ...
    if config.METRICS_PORT is not None:
        if config.METRICS_LISTEN_ADDR_IPV4:
            task = asyncio.start_server(
                handle_metrics, config.METRICS_LISTEN_ADDR_IPV4, config.METRICS_PORT
            )
            servers.append(loop.run_until_complete(task))
        if config.METRICS_LISTEN_ADDR_IPV6 and socket.has_ipv6:
            task = asyncio.start_server(
                handle_metrics, config.METRICS_LISTEN_ADDR_IPV6, config.METRICS_PORT
            )
            servers.append(loop.run_until_complete(task))
